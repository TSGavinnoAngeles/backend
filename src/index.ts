...
app.get("/users", async (req, res) => {
  try {
    ...
    const usersData = users.map(
      (user: { _id: any; email: any; password: any }) => ({
        _id: user._id,
        email: user.email,
...
app.get("/cards", async (req, res) => {
  try {
    ...
    const cardsData = cards.map(
      (card: { bal: any; idNums: any; stat: any }) => ({
        bal: card.bal,
        idNums: card.idNums,
        stat: card.stat,
      })
  ...
  try {
    ...
    if (isAddition) {
      ...
      if (Number(card.bal) - updatedValue < 0) {
        return res
          .status(400)
          .json({ error: "Cannot deduct beyond current balance" });
      }
...
app.post("/addStat", async (req, res) => {
  const { coordinates, stationId, label, position, active, connecting } =
    req.body;
  if (
    coordinates === null ||
    stationId === null ||
